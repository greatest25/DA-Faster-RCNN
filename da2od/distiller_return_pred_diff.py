...
from da2od.utils import SaveIO, ManualSeed, ReplaceProposalsOnce, set_attributes
...
class DA2ODDistiller(Distiller):
    ...
    def from_config(cls, cfg, teacher, student):
        return DA2ODDistiller(teacher, student,
                        ...
                        do_cls_dst=cfg.DA.DISTILL.ROIH_CLS_ENABLED, 
                        do_obj_dst=cfg.DA.DISTILL.OBJ_ENABLED,
                        do_rpn_reg_dst=cfg.DA.DISTILL.RPN_REG_ENABLED,
                        do_roih_reg_dst=cfg.DA.DISTILL.ROIH_REG_ENABLED,
    ...
    def register_hooks(self):
        self.student_rpn_io, self.student_rpn_head_io, self.student_boxpred_io, self.student_boxhead_io = SaveIO(), SaveIO(), SaveIO(), SaveIO()
        self.teacher_backbone_io, self.teacher_rpn_head_io, self.teacher_boxpred_io, self.teacher_anchor_io, self.teacher_boxhead_io = SaveIO(), SaveIO(), SaveIO(), SaveIO(), SaveIO()
    ...
    def __call__(self, teacher_batched_inputs, student_batched_inputs):
        ...
        # compute the prediction differences between roi_head pred
        ...
        diff = student_cls_logits - teacher_cls_logits
        diff = diff * diff
        
        diff = torch.mean(diff, dim=1, keepdim=True)
        
        pseudo_boxes = list()
        for input in student_batched_inputs:
            pseudo_boxes.append(input['instances'].gt_boxes.tensor.cuda()) # Boxes数据结构
...
# Any modifications to the torch module itself go here and are mixed in in rcnn.DA2OD
